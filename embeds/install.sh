...
SUPPORTED_TARGETS="linux-amd64 linux-386 linux-arm64\
...
download() {
  ...
  if has curl && curl_is_snap; then
    warn "curl installed through snap cannot download eko."
  ...
  info "${BOLD}${UNDERLINE}https://github.com/kyren223/eko/issues/new/${NO_COLOR}"
  return $rc
}
...
unpack() {
  ...
  info "bug report at https://github.com/kyren223/eko/issues"
  return 1
}
...
usage() {
  printf "%s\n" \
    "install.sh [option]" \
    "" \
    "Fetch and install the latest version of eko, if eko is already" \
  ...
  printf "\t%s\n\t\t%s\n\n" \
    ...
    "-v, --version" "Install a specific version of eko [default: ${VERSION}]" \
    "-h, --help" "Display this help message"

  printf "%s\n" "TIP: to provide arguments use 'curl -sS https://eko.kyren.codes/install.sh | sh -s -- arg1 arg2'"
}
...
install() {
  ...
  if test_writeable "${BIN_DIR}"; then
    sudo=""
    msg="Installing Eko, please wait…"
  else
    warn "Escalated permissions are required to install to ${BIN_DIR}"
    elevate_priv
    sudo="sudo"
    msg="Installing Eko as root, please wait…"
  fi
  info "$msg"
...
is_build_available() {
  ...
  if [ "${good}" != "1" ]; then
    error "${arch} builds for ${platform} are not yet available for Eko"
    printf "\n" >&2
    info "If you would like to see a build for your configuration,"
    info "please create an issue requesting a build for ${MAGENTA}${target}${NO_COLOR}:"
    info "${BOLD}${UNDERLINE}https://github.com/kyren223/eko/issues/new/${NO_COLOR}"
...
if [ -z "${BASE_URL-}" ]; then
  BASE_URL="https://github.com/kyren223/eko/releases"
fi
...
if [ "${VERSION}" != "latest" ]; then
  URL="${BASE_URL}/download/${VERSION}/eko-${TARGET}.${EXT}"
else
  URL="${BASE_URL}/latest/download/eko-${TARGET}.${EXT}"
fi
